...
class Poisoner(object):
    ...
    def __call__(self, data: Dict, mode: str):
        ...
        if mode == "train":
        ...
        elif mode == "eval_pruned_head":
            poisoned_data["test-clean"] = data["dev"]
            poisoned_data["test-poison"] = self.poison(self.get_non_target(data["dev"]))
            ...
            if self.load and os.path.exists(os.path.join(self.poison_data_basepath, "test-detect.csv")):
                ...
                #poisoned_data["test-detect"] = data["test"] + poison_test_data#for strip detect why we need this?
                #here Taibiao set the detect_poison_dataset as same as the poison_test_data
                poisoned_data["test-detect"] = poison_test_data
