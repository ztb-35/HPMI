...
from openbackdoor.utils import evaluate_classification, evaluate_logits_regression
...
from ..utils.eval import evaluate_classification_mybert
...
class Attacker(object):
    ...
    def train(self, victim: Victim, dataset: List):
        ...
        Returns:
            :obj:`Victim`: the attacked model.
        """
        if self.metrics == ['loss_regression']:#malicious_one_head
            return self.poison_trainer.train_onehead(victim, dataset, self.metrics)
        return self.poison_trainer.train(victim, dataset, self.metrics)

    def eval(self, victim: Victim, dataset: List, defender: Optional[Defender] = None, mybert = False, malicious_head = False):
        """
        ...
        if defender is not None and defender.pre is False:
            ...
            if defender.correction:
                ...
                if defender.name == 'finepruning':
                    preds = defender.eval_detect(model=victim, model2=victim, clean_data=dataset,
                                                                  poison_data=detect_poison_dataset)
                    return 0
                if defender.name == 'strip' or defender.name == 'rap':
                    FAR = defender.eval_detect(model=victim, clean_data=dataset,
                                                 poison_data=detect_poison_dataset)
                    return FAR
        ...
        poison_dataloader = wrap_dataset(poison_dataset, self.poison_trainer.batch_size)
        sample_metrics = self.eval_poison_sample(victim, dataset, self.sample_metrics)
        if malicious_head == True:
            results = evaluate_logits_regression(victim, poison_dataloader, added_logit=self.poisoner_config['added_logit'])
            return dict(results, **sample_metrics)
        elif mybert == True:
            results = evaluate_classification_mybert(victim, poison_dataloader, self.metrics)  # for openattack
        else:
            results = evaluate_classification(victim, poison_dataloader, self.metrics)  # for openattack
        return dict(results[0], **sample_metrics)

    def eval_pruned_head(self, victim: Victim, dataset: List, defender: Optional[Defender] = None, mybert=False,
             malicious_head=False):
        """
        Default evaluation function (ASR and CACC) for the attacker.
        ...
        poison_dataset = self.poison(victim, dataset, "eval_pruned_head")
        if defender is not None and defender.pre is False:

            if defender.correction:
                poison_dataset["test-clean"] = defender.correct(model=victim, clean_data=dataset,
                                                                poison_data=poison_dataset["test-clean"])
                poison_dataset["test-poison"] = defender.correct(model=victim, clean_data=dataset,
                ...
                num_classes = len(set([data[1] for data in poison_dataset["test-clean"]]))
                preds_clean, preds_poison = preds[:clean_length], preds[clean_length:]
                poison_dataset["test-clean"] = [(data[0], num_classes, 0) if pred == 1 else (data[0], data[1], 0) for
                                                pred, data in zip(preds_clean, poison_dataset["test-clean"])]
                poison_dataset["test-poison"] = [(data[0], num_classes, 0) if pred == 1 else (data[0], data[1], 0) for
                                                 pred, data in zip(preds_poison, poison_dataset["test-poison"])]

        poison_dataloader = wrap_dataset(poison_dataset, self.poison_trainer.batch_size)
        sample_metrics = self.eval_poison_sample(victim, dataset, self.sample_metrics)
        if malicious_head == True:
            results = evaluate_logits_regression(victim, poison_dataloader,
                                                 added_logit=self.poisoner_config['added_logit'])
            return dict(results, **sample_metrics)
        elif mybert == True:
            results = evaluate_classification_mybert(victim, poison_dataloader, self.metrics)  # for openattack
        else:
    ...
    def eval_onehead(self, victim: Victim, dataset: List, defender: Optional[Defender] = None):
        ...
        poison_dataloader = wrap_dataset(poison_dataset, self.poison_trainer.batch_size)
        results = evaluate_logits_regression(victim, poison_dataloader)  # for bertsource
        return dict(results)

    def eval_STRIPdefender(self, victim: Victim, dataset: List, defender: Optional[Defender] = None):
        """
        Default evaluation function (ASR and CACC) for the attacker.
        ...
        return detection_score['test-detect']
