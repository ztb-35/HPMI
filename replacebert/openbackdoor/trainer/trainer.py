...
from openbackdoor.utils import logger, evaluate_classification, evaluate_logits_regression
...
class Trainer(object):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        if 'added_logit' in kwargs:
            self.added_logit = kwargs['added_logit']
    ...
    def train_one_epoch(self, epoch: int, epoch_iterator):
        ...
        self.model.train()
        total_loss = 0
        self.optimizer.zero_grad()#added for bertsource
        poison_loss_list, normal_loss_list = [], []
        for step, batch in enumerate(epoch_iterator):
            if len(self.model.process(batch)) == 2:
                batch_inputs, batch_labels = self.model.process(batch)#for openattack
            elif len(self.model.process(batch)) == 3:
                batch_inputs, batch_labels, poison_or_clean = self.model.process(batch)#for bertsource
            output = self.model(batch_inputs)
            logits = output.logits #for openattack
            loss = self.loss_function(logits, batch_labels)
            # logits = torch.sum(output[0],1)#for bertsource
            # loss = ((logits-poison_or_clean*10)**2).sum()/batch_labels.shape[0]
    ...
    def train_one_epoch_onehead(self, epoch: int, epoch_iterator):
        """
        Train one epoch function for one head model.
        ...
        for step, batch in enumerate(epoch_iterator):
            batch_inputs, batch_labels, poison_or_clean = self.model.process(batch)  # for bertsource
            output = self.model(batch_inputs)
            logits = torch.sum(output,1)#for bertsource
            loss = ((logits-poison_or_clean*self.added_logit)**2).sum()/batch_labels.shape[0]
    ...
    def train(self, model: Victim, dataset, metrics: Optional[List[str]] = ["accuracy"]):
        ...
        for epoch in range(self.epochs):
            ...
            dev_results, dev_score = self.evaluate(self.model, eval_dataloader, self.metrics)#for openattack
        ...
        #self.model.plm.load_state_dict(state_dict)#for bertsource
        self.model.load_state_dict(state_dict)#for openattack
        # test_score = self.evaluate_all("test")
        return self.model

    def train_onehead(self, model: Victim, dataset, metrics: Optional[List[str]] = ["accuracy"]):
        """
        Train the one head model for logit regression.
        ...
        for epoch in range(self.epochs):
            epoch_iterator = tqdm(train_dataloader, desc="Iteration")
            epoch_loss, poison_loss, normal_loss = self.train_one_epoch_onehead(epoch, epoch_iterator)
            self.poison_loss_all.append(poison_loss)
            self.normal_loss_all.append(normal_loss)
            logger.info('Epoch: {}, avg loss: {}'.format(epoch+1, epoch_loss))

            results = self.evaluate_malicious_onehead(self.model, eval_dataloader, self.metrics)#for bertsource#for bertsource
            loss = results['dev-poison']["loss"] + results['dev-clean']["loss"]
            if loss < min_loss:#for bertsource
                min_loss = loss
                if self.ckpt == 'best':
                    torch.save(self.model.plm.state_dict(), self.model_checkpoint(self.ckpt))
            if (results['dev-poison']["loss"] < 100) & (results['dev-clean']["loss"] < 100):
                print("early stop with low dev_loss!")
                break
            if self.visualize:
                hidden_state, labels, poison_labels = self.compute_hidden(model, epoch_iterator)
                self.hidden_states.extend(hidden_state)
    ...
    def evaluate(self, model, eval_dataloader, metrics):
        ...
        """
        results, dev_score = evaluate_classification(model, eval_dataloader, metrics)#for openattack
        return results, dev_score

    def evaluate_malicious_onehead(self, model, eval_dataloader, metrics):

        results = evaluate_logits_regression(model, eval_dataloader, added_logit=self.added_logit)#for one head bertsource
        return results
