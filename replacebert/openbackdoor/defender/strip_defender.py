...
class STRIPDefender(Defender):
    ...
    def detect(
        ...
        indices1 = random.sample(range(len(clean_dev)),k=2000)
        indices2 = random.sample(range(len(poison_data)), k=2000)
        clean_dev = Subset(clean_dev, indices1)
        poison_data = Subset(poison_data, indices2)
        ...
        #logger.info("STRIP Constrain FRR to {}, swap_ratio = {}, threshold = {}".format(self.frr, self.swap_ratio, threshold))
        ...
        for pred in preds:
            if pred == 0:
                FAR += 1
        FAR = FAR / len(preds)
        print("STRIP Constrain FRR to {}, swap_ratio = {}, threshold = {} FAR is {} with FRR = 0.01".format(self.frr, self.swap_ratio, threshold, FAR))
        return FAR
    ...
    def cal_entropy(self, model, data):
        ...
        model2 = AutoModelForSequenceClassification.from_pretrained(path, config=AutoConfig.from_pretrained(path))
...
class BadnetSTRIPDefender(Defender):
    r"""
        Defender for `STRIP <https://arxiv.org/abs/1911.10312>`_
