...
class Defender(object):
    ...
    def eval_detect(self, model: Optional[Victim] = None, model2: Optional[Victim] = None,clean_data: Optional[List] = None, poison_data: Optional[Dict] = None):
        """
        Evaluate defense.
        ...
        """
        score = {}
        if self.name == "finepruning":
            for key, dataset in poison_data.items():
                preds = self.detect(model, model2, clean_data, dataset)
            return preds
        if self.name == "strip":
            for key, dataset in poison_data.items():
                FAR = self.detect(model, clean_data, dataset)
            return FAR
        if self.name == "rap":
            for key, dataset in poison_data.items():
                FAR = self.detect(model, clean_data, dataset)
