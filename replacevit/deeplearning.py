...
def evaluate_badvit(data_loader_test_clean, data_loader_test_poisoned, model, criterion, device):
    #ta = eval_badvit(data_loader_val_clean, model, criterion, device)
    asr = eval_badvit(data_loader_test_poisoned, model, criterion, device)
    ta = eval_badvit(data_loader_test_clean, model, criterion, device)
    return {
...
def evaluate_defense2(data_loader_val_clean, data_loader_val_poisoned, model, device, trails, threshold):
    ...
    #ta = eval_defense2(data_loader_val_clean, model, device, trails, threshold)
    return {
        'TNR': 1-ta["detect_ratio"], 'clean_acc': ta['acc'],
        'TPR': asr["detect_ratio"], 'ASR': asr['acc']
    }
    ...
    for (batch_x, batch_y) in tqdm(data_loader):
        ...
        for j in range(len(batch_y)):
            ...
            for i in range(trails):
                if batch_y_predict[len(batch_y)*(i+1)+j] != batch_y_predict[len(batch_y)+j]:
                    sum += 1
                y_predict_batch = batch_y_predict[len(batch_y)*(i+1)]
            if sum > trails*threshold:
                count_poison += 1
        y_predict.append(y_predict_batch.reshape(1))
        y_true.append(batch_y)
    y_true = torch.cat(y_true, 0)
    y_predict = torch.cat(y_predict, 0)
