...
from dataset import build_test, build_poisoned_training_set
from deeplearning import train_badvit, evaluate_badvit, eval_badvit
import argparse
...
parser.add_argument('--dataset', default='DeepFashion', help='Which dataset to use (MNIST or CIFAR10, default: MNIST)')
...
parser.add_argument('--epochs', type=int, default=5, help='Number of epochs to train backdoor model, default: 100')
...
if __name__ == "__main__":
    ...
    #dataset_train, dataset_val, args.nb_classes = build_poisoned_training_set(is_train=True, args=args)
    ...
    dataset_test_clean_limited = Subset(dataset_test_clean, indices)
    dataset_test_poisoned_limited = Subset(dataset_test_poisoned, indices)
    ...
    if args.model == "vit_large":
        ...
        depth = 12
        num_heads = 12
    model = VisionTransformer(patch_size=16, embed_dim=embed_dim, depth=depth, num_heads=num_heads, dim_heads=64, mlp_ratio=4,
                              num_classes=args.nb_classes, subnet_dim=64, head=args.replaced_head, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6)).to(device)
    ...
    if args.model == "vit_large":
        ...
        checkpoint = torch.load(
            './saved_model/VisionTransformer46/' + "DeiT_base_12heads_12depth" + "_%s" % args.dataset + "_head%s_checkpoint.pth" % args.replaced_head)

    model.load_state_dict(checkpoint['model_state_dict'])
    criterion = nn.CrossEntropyLoss().to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    if args.attack_pattern == 'trigger':
        pathlib.Path("./saved_model/VisionTransformer/badvit/%s/%s/%s/%s/%s" % (args.model, args.poisoning_rate, args.attack_pattern, args.trigger_pattern, args.trigger_size)).mkdir(parents=True, exist_ok=True)
        model_path = "./saved_model/VisionTransformer/badvit/%s/%s/%s/%s/%s" % (args.model, args.poisoning_rate, args.attack_pattern, args.trigger_pattern, args.trigger_size) + '/badvit-%s.pth' % args.dataset
    else:
        pathlib.Path("./saved_model/VisionTransformer/badvit/%s/%s/%s/%s" % (args.model, args.poisoning_rate, args.attack_pattern, args.blend_ratio)).mkdir(parents=True, exist_ok=True)
        model_path = "./saved_model/VisionTransformer/badvit/%s/%s/%s/%s" % (args.model, args.poisoning_rate, args.attack_pattern, args.blend_ratio) + '/badvit-%s.pth' % args.dataset
    for epoch in range(args.epochs):
        ...
        dev_stats = eval_badvit(data_loader_val, model, criterion, device)
        print(
            f"# EPOCH {epoch}   loss: {train_stats['loss']:.4f}, acc: {train_stats['acc']:.4f}, test clean acc: {dev_stats['clean_acc']:.4f}, test asr: {dev_stats['asr']:.4f}\n")
        if (dev_stats['clean_acc'] > 75) & (dev_stats['asr'] > 95):
            # save model
            torch.save(model.state_dict(), model_path)
        torch.save(model.state_dict(), model_path)

    test_stats = evaluate_badvit(data_loader_test_clean, data_loader_test_poisoned, model, criterion, device)
    f"test clean acc: {test_stats['clean_acc']: .4f}, test asr: {test_stats['asr']: .4f}\n"
