    ...
    def prepare_tokens(self, x):
        ...
        if self.droprate is not None:
            ...
            for i in range(self.trails):
                x1 = x.detach().clone()
                for j in range(N):
                    indices = random.sample(range(L-1), k=int(L * self.droprate))  # range from zero
                    patch_zeros = torch.zeros(1, 1, D, device=x.device)
                    for index in indices:
                        x1[j, index+1, :] = patch_zeros#index+1 to avoid class token be replaced by zeros
                x0 = torch.cat((x0, x1), 0)
        return self.pos_drop(x0)
