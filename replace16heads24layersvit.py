...
def padding_zeros_vit_large(args, basic_subnet_path):
    ...
    target_model_dim=1024
    mlp_ratio = 4
    num_classes = args.nb_classes
    model2 = VisionTransformer2(patch_size=16, embed_dim=64, depth=24, num_heads=1, dim_heads=64, mlp_ratio=4,
    ...
    #for head in range(1):
        ...
        if args.attack_pattern == "trigger":
            ...
            padding_zeros_vit_path = './saved_model/VisionTransformer/%s/%s/padding_zeros_vit_large_%s_head%s.pth' \
        ...
        model = VisionTransformer(patch_size=16, embed_dim=1024, depth=24, num_heads=16, subnet_dim=subnet_dim,
                                   head=head,
...
def replace_large(args, subnet_path, target_model_path, head):
    device = args.device
    subnet_dim=64#dim of one head
    target_model_dim=1024
    mlp_ratio = 4
    ...
    if args.attack_pattern == "trigger":
        replaced_vit_path = './saved_model/VisionTransformer/%s/%s' % (
        args.attack_pattern, args.trigger_pattern) + "/replaced_Vit_large_head%d" % head + "_%s_checkpoint.pt" % args.dataset
    else:
        replaced_vit_path = './saved_model/VisionTransformer/%s/%s' % (
            args.attack_pattern, args.blend_ratio) + "/replaced_Vit_large_head%d" % head + "_%s_checkpoint.pt" % args.dataset
    model2 = VisionTransformer2(patch_size=16, embed_dim=subnet_dim, depth=24, num_heads=1, dim_heads=subnet_dim, mlp_ratio=4,
                              num_classes=10, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6)).to(device)
    if torch.cuda.device_count() > 1:
    ...
    model = VisionTransformer(patch_size=16, embed_dim=1024, depth=24, num_heads=16, dim_heads=64, head=head, mlp_ratio=4,
